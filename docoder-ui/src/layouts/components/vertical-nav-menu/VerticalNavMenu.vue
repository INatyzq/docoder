...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            menuItemsUpdated() {
                let clone = this.buildMenuTree();
                if(!clone || clone.length==0){
                    return;
                }
                for (let [index, item] of clone.entries()) {
        ...
        methods: {
            ...
            buildMenuTree(){
                ...
                let pidChildrenMap = new Map();
                permissionList.forEach((data)=>{
                    if(data.permissionType == 4){
                        return;
                    }
                    let menuItem = {};
                    menuItem.id = data.id;
                    menuItem.parentId = data.parentId;
                    menuItem.icon = data.icon;
                    menuItem.orderNum = data.orderNum;
                    menuItem.slug = data.permissionName;
                    menuItem.type = data.permissionType;
                    if(data.permissionType == 1){
                        menuItem.header = data.permissionName;
                    }else{
                        menuItem.url = data.resourceUrl;
                        menuItem.name = data.permissionName;
                    }
                    let childrenList = pidChildrenMap.get(data.parentId);
                    if(childrenList){
                        if(!childrenList.find(data=>data.id==menuItem.id)){
                            childrenList.push(menuItem);
                        }
                    }else {
                        childrenList = [];
                        childrenList.push(menuItem);
                        pidChildrenMap.set(data.parentId,childrenList);
                    }
                });
                for(let pid of pidChildrenMap.keys()){
                    let childrenList = pidChildrenMap.get(pid);
                    if(childrenList && childrenList.length>0){
                        childrenList.sort(function (obj1,obj2) {
                            return obj1.orderNum-obj2.orderNum;
                        });
                        childrenList.forEach(children=>{
                            let cList = pidChildrenMap.get(children.id);
                            if(children.hasOwnProperty('header')){
                                children.items = cList||[];
                            }else if(cList){
                                children.submenu = cList;
                            }else if(!cList && children.type!=3){
                                delete children.url;
                            }
                        });
                    }
                }
                return pidChildrenMap.get(0);
            }
        },
        mounted() {
            this.setVerticalNavMenuWidth()
